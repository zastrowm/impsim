...
/**
 * Class for instantiating the data in an ZSlide joint to be something other than the default values. The execute method
 ...
 * @edit ebertb Fixed bug where this class acted exactly like the DataXSlideCommand
 * @version 1.01
 */
public class DataZSlideCommand extends DataPrismCommand {
	 ...
	 *             malfunction.
	 */
	public DataZSlideCommand(Point origin, String jointName) throws NullParameterException, InvalidParameterException {
		super(origin, new Point(notNull(origin, 0).getX(), notNull(origin, 0).getY(), notNull(origin, 0).getZ()+1), new Point(
				notNull(origin, 0).getX()+1, notNull(origin, 0).getY(), notNull(origin, 0).getZ()), jointName);
	}
	...
	public boolean equals(Object otherObject) {
		...
		if (!(otherObject instanceof DataZSlideCommand)) {
			return false;
		} else {
			DataZSlideCommand otherCmd = (DataZSlideCommand) otherObject;
	...
	public String toString() {
		return "DataZSlideCommand [name=" + this.name + ", origin=" + this.origin + ", commonAxis=" + this.commonAxis + ", halfPlane=" + this.halfPlane + "]";
	}
}
