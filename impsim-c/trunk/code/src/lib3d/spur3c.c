...
|
|     Access:
|       (PFV) spur3c(real x[], real y[], real z[], int ip, int iq,
...
|     real   z[],   i-Array of z(x,y) values to be contoured.
|     int    ip,    i-Row    dimension of the z(x,y) grid.
|     int    iq,    i-Column dimension of the z(x,y) grid.
|     double height,i-The z value for which contour is desired.
...
#ifdef ANSI
  void spur3c(real x[], real y[], real z[], int ip, int iq,
    double height, int lopen, int i0, int j0, int iinc0, int jinc0)
#else
  void spur3c(x, y, z, ip, iq, height, lopen, i0, j0, iinc0,jinc0)
  real   x[], y[], z[];
  int    ip, iq;
  double height;
...
/*   happens are found using inverse interpolation. */
  ...
  xx = 0.5 * (x[i] * (1.0 - t) + x[i+iinc+jinc] * (1.0 + t));
  yy = 0.5 * (y[j] * (1.0 - t) + y[j+jinc-iinc] * (1.0 + t));
  drwa3c(xx, yy, height, 1.0);
