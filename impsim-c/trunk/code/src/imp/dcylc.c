...
|     of a cylindric (CYLINDER) joint.  pt0 is the local origin, pt1 is
|     a point on the local w and w' axes, and pt2 is a point on the
|     local u and u' axes of the joint.
|
|     Access:
|       (void) dcylc(Word name, Token pt0[3],Token pt1[3],Token pt2[3]);
|     Word name,i-Name of the cylindric (CYLINDER) joint.
...
#endif
{
  ...
  echoc();
  textc("*** There is no cylinder joint named ", FALSE);
  textc(name, FALSE);
  textc(". ***", TRUE);
  System.nerror = 3;
  return;
