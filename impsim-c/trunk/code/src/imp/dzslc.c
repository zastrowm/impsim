...
|     of a ZSLIDE joint.  pt0 is the local origin.
|
|     Access:
|       (void) dzslc(Word name, Token pt0[3]);
|     Word name,i-Name of the ZSLIDE joint.
...
#endif
{
  ...
  echoc();
  textc("*** There is no zslide joint named ", FALSE);
  textc(name, FALSE);
  textc(". ***", TRUE);
  System.nerror = 3;
  return;
