...
* ------------------------------------------------------------------- */
/* ------------------------------------------------------------------ */
/* dcamc-01 Jan 00:25 Jun 09
|     (C)Copyright, JJU Research, 2000-2009.
|
|     Description:
|       Store pointer to the pitch curve for a specified CAM joint.
|
|     Access:
|       (void) dcamc(Word name, Token curveu, Token curvev);
|     Word name,i-Name of the CAM joint to be set.
...
|
|     Error Conditions:
|       System.nerror = 3 indicates an undefined CAM joint name.
...
/* Locate the appropriate CAM joint. */
...
/* Store the data. */
            ...
            if (jvar->jvrVup || jvar->jvrVvp) {
              echoc();
              textc("*** Joint ", FALSE);
              textc(name, FALSE);
              textc(" cam shape is redefined. ***", TRUE);
            }
...
/* Print error messages. */
  ...
  echoc();
  textc("*** There is no CAM joint named '", FALSE);
  textc(name, FALSE);
  textc("'. ***", TRUE);
  System.nerror = 3;
  return;
